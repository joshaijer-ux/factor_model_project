...
def construct_variables(df):
    ...
    # FIX: chpmia — calendar-month-accurate 12-month lag, then LOO industry-adjust
    id_col = 'gvkey' if df['PERMNO'].isnull().all() else 'PERMNO'
    df = df.sort_values([id_col, 'DATE'])
    lag12 = df[[id_col, 'DATE', 'chpmia']].copy()
    lag12['DATE'] = lag12['DATE'] + pd.DateOffset(months=12)
...
def Rankise(market):
    ...
    raw_data['gvkey'] = raw_data['gvkey'].astype(str).str.zfill(6)
    ...
    df = df.groupby('DATE', group_keys=False).apply(rank_all)
